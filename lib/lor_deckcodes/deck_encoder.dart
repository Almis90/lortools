...
import 'card_code_and_count.dart';
import 'varint_translator.dart';

class DeckEncoder {
  static const int cardCodeLength = 7;
  static const int maxKnownVersion = 5;
  static const int initialVersion = 1;
  static const int format = 1;

  Map<String, int> factionCodeToIntIdentifier = <String, int>{};
  Map<int, String> intIdentifierToFactionCode = <int, String>{};
  Map<String, int> factionCodeToLibraryVersion = <String, int>{};
  ...
  List<List<CardCodeAndCount>> sortGroupOf(
    ...
    for (var i = 0; i < groupOf.length; i++) {
      ...
      tmp.sort((a, b) {
        return a.cardCode
            .substring(4, a.cardCode.length)
            .compareTo(b.cardCode.substring(4, b.cardCode.length));
      });
  ...
  List<List<CardCodeAndCount>> getGroupedOfs(List<CardCodeAndCount> list) {
    ...
    for (var item in list) {
      int factionCode =
          factionCodeToIntIdentifier.containsKey(getCardFaction(item.cardCode))
              ? factionCodeToIntIdentifier[getCardFaction(item.cardCode)]!
              : -1;
      if (factionCode == -1) {
        throw Exception("Card code invalid.");
      }
  ...
  int getMinSupportedLibraryVersion(List<CardCodeAndCount> deck) {
    ...
    var codes =
        deck.map<String>((ccc) => ccc.cardCode.substring(2, 4)).toList();
    var libCodes = codes
        .map((factionCode) =>
            factionCodeToLibraryVersion.containsKey(factionCode)
                ? factionCodeToLibraryVersion[factionCode]!
                : maxKnownVersion)
        .toList();
